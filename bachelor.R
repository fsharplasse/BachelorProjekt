...
choice_sub <- choice %>% group_by(Year, Fuel) %>% summarise(AverageWeight = mean(Weight))

ggplot(choice_sub,aes(x=Year,y=AverageWeight,color=Fuel))+
  xlab("Registration Year") +
  ylab("Weight in Kg")+
  geom_line()

choice_sub1 <- choice %>% group_by(Year, Fuel) %>% summarise(AverageSize = mean(Size))

ggplot(choice_sub1,aes(x=Year,y=AverageSize,color=Fuel))+
  xlab("Registration Year") +
  ylab("Size in cubic metres")+
  geom_line()

choice_sub2 <- choice %>% group_by(Year, Fuel) %>% summarise(AverageEE = mean(EngineEffect))

ggplot(choice_sub2,aes(x=Year,y=AverageEE,color=Fuel))+
  xlab("Registration Year") +
  ylab("Engine Effect (kW)")+
  geom_line()

choice_sub3 <- choice %>% group_by(Year, FuelSize) %>% summarise(AveragePrice = mean(Prices))

ggplot(choice_sub3,aes(x=Year,y=AveragePrice,color=FuelSize))+
  xlab("Registration Year") +
  ylab("Price")+
  geom_line()
